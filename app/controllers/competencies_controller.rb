class CompetenciesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @competency.save
        format.html { redirect_to @competency, :notice => t('successfully_created', :model => t('competency.gender', :count => 1)) }
  ...
  def update
    ...
    respond_to do |format|
      if @competency.update_attributes(params[:competency])
        format.html { redirect_to @competency, :notice => t('successfully_updated', :model => t('competency.gender', :count => 1)) }
